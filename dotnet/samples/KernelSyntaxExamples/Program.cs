...
public static class Program
{
    ...
    {
        await Console.Out.WriteLineAsync("请输入要启动的案例编号：");
        string input = Console.ReadLine();
        string formattedInput = $"{int.Parse(input):D2}"; // 输出 "03"

        var examples = (Assembly.GetExecutingAssembly().GetTypes())
            .Where(type => type.Name.StartsWith($"Example{formattedInput}", StringComparison.OrdinalIgnoreCase))
